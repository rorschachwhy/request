...
class TestInterfaceCase(ParametrizedTestCase):
    ...
    def test_POST(self):
        ...
        self.db1_cursor.execute(
            'SELECT request_url, request_param FROM test_data WHERE case_id = %s', (self.test_data.case_id,))
        ...
        response = self.http.post(
            self.test_data.request_url,  str(self.test_data.request_param))
        ...
        # 更新结果表中的用例运行结果
        try:
            self.db1_cursor.execute('UPDATE test_result SET request_param = %s WHERE case_id = %s', (str(
                self.test_data.request_param), self.test_data.case_id))
            self.db1_cursor.execute('UPDATE test_result SET result = %s WHERE case_id = %s', (
                self.test_data.result, self.test_data.case_id))
    ...
    def test_login_normal(self):
        ...
        response = self.http.get(
            self.test_data.request_url,  self.test_data.request_param)
        if {} == response:
            self.test_data.result = 'Error'
            try:
                # 更新结果表中的用例运行结果
        ...
        try:
            ...
            self.db2_cursor.execute('SELECT user_id FROM 1dcq_user WHERE mobile = %s', (eval(
                self.test_data.request_param)['mobile'],))
            ...
            self.assertEqual(response['data'][
                             'nikeName'], None, msg='nikeName错误')
            self.assertEqual(response['data'][
                             'cityName'], None, msg='cityName错误')
            self.assertEqual(response['data']['userId'],
                             user_id, msg='userId错误')  # 2910057590
            self.assertEqual(response['data'][
                             'cityName'], None, msg='cityName错误')
            self.assertEqual(response['data'][
                             'payPasswordFlag'], 1, msg='payPasswordFlag错误')
            self.assertEqual(response['data'][
                             'imgSmall'], None, msg='imgSmall错误')
    ...
    def test_chpasswd_normal(self):
        ...
        # 步骤1-登录
        self.db1_cursor.execute(
            'SELECT request_url, request_param FROM pre_condition_data WHERE case_id = %s and step=1', (self.test_data.case_id,))
        temp_result = self.db1_cursor.fetchone()
        request_url = temp_result[0]
        request_param = temp_result[1]
        lgin_response = self.http.get(request_url, request_param)
